...
import { toISODate, monthStart, toDateString } from '../lib/date'

type Category = { id: string; name: string; color: string | null }
type Expense = { id: string; amount: number; memo: string | null; spent_on: string; category_id: string | null; categories: { name: string; color: string | null } | null }
...
export default function HistoryPage() {
  ...
  async function load(month: string) {
    ...
    const { data } = await supabase
      .from('expenses')
      .select('id,amount,memo,spent_on,category_id,categories(name,color)')
      .gte('spent_on', month)
      .lt('spent_on', nextISO)
  ...
  function csvExport() {
    ...
    for (const e of expenses) rows.push([e.spent_on, e.categories?.name ?? '', String(e.amount), e.memo ?? ''])
  ...
  return (
    <div>
      <div className="hrow" style={{ marginBottom: 12, paddingTop: 4 }}>
        ...
        <button className="btn" style={{ fontSize: 12 }} onClick={csvExport} disabled={expenses.length === 0}>CSV出力</button>
      </div>
      ...
      <div className="card">
        ...
        {!loading && expenses.length === 0 && <p style={{ color: '#94a3b8', fontSize: 13 }}>この月の支出はありません</p>}
        {expenses.map((e, i) => {
          ...
          return (
            <React.Fragment key={e.id}>
              {showDate && (
                <div style={{ fontSize: 12, color: '#64748b', fontWeight: 600, padding: '10px 0 4px', borderTop: i === 0 ? 'none' : '1px solid #1e293b' }}>
                  {e.spent_on.slice(5).replace('-', '月')}日（{['日','月','火','水','木','金','土'][new Date(e.spent_on + 'T00:00:00').getDay()]}）
                </div>
              )}
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderTop: showDate ? 'none' : '1px solid #0f172a' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div className="hrow" style={{ gap: 6 }}>
                    ...
                    <span style={{ fontSize: 11, color: '#94a3b8', background: '#0f172a', padding: '1px 8px', borderRadius: 999, whiteSpace: 'nowrap' }}>
                      {e.categories?.name ?? '未分類'}
                    </span>
                  </div>
                  {e.memo && <p style={{ fontSize: 12, color: '#64748b', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{e.memo}</p>}
                </div>
                <button className="btn" style={{ fontSize: 11, padding: '3px 8px', flexShrink: 0 }} onClick={() => openEdit(e)}>編集</button>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, zIndex: 200 }}>
          <div className="card" style={{ width: '100%', maxWidth: 360, margin: 0 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                <label>金額</label>
                <input className="input" type="number" inputMode="numeric" value={editAmount} onChange={e => setEditAmount(e.target.value)} />
              </div>
              <div>
                <label>日付</label>
